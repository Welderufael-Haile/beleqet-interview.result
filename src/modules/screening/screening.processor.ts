...
export class ScreeningProcessor {
  ...
  constructor(
    ...
    if (apiKey) {
      ...
      this.logger.warn('OPENAI_API_KEY not set. AI screening will use fallback scoring.');
    }
  }
...
`;
    ...
    try {
      if (!this.openai) {
        return this.getFallbackScore('AI scoring unavailable - OPENAI_API_KEY is not set.');
      }
      ...
      return this.getFallbackScore('AI scoring unavailable - manual review required.');
    }
  }

  private getFallbackScore(reasoning: string): AiScoreResult {
    return {
      overallScore: 50,
      skillScore: 50,
      experienceScore: 50,
